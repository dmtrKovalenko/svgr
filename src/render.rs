...
fn apply_viewbox_transform(
    ...
    let ts = usvgr::utils::view_box_to_transform(view_box.rect, view_box.aspect, img_size.to_size());
    canvas.apply_transform(ts.to_native());
}
...
fn render_group_impl(
    ...
    if g.opacity != usvgr::Opacity::ONE {
