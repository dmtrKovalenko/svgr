...
fn process() -> Result<(), String> {
    ...
    let xml_tree = timed(args.perf, "XML Parsing", || {
        let xml_opt = usvgr::roxmltree::ParsingOptions {
            allow_dtd: true,
            ..Default::default()
        };
        usvgr::roxmltree::Document::parse_with_options(svg_string, xml_opt)
...
const HELP: &str = "\
svgr is an SVG rendering application.

USAGE:
  svgr [OPTIONS] <in-svg> <out-png>  # from file to file
  svgr [OPTIONS] <in-svg> -c         # from file to stdout
  svgr [OPTIONS] - <out-png>         # from stdin to file
  svgr [OPTIONS] - -c                # from stdin to stdout

  svgr in.svg out.png
  svgr -z 4 in.svg out.png
  svgr --query-all in.svg
...
fn parse_args() -> Result<Args, String> {
    ...
    let mut default_size = usvgr::Size::from_wh(100.0, 100.0).unwrap();
    if let (Some(w), Some(h)) = (args.width, args.height) {
        default_size = usvgr::Size::from_wh(w as f32, h as f32).unwrap();
        fit_to = FitTo::Size(w, h);
    } else if let Some(w) = args.width {
        default_size = usvgr::Size::from_wh(w as f32, 100.0).unwrap();
        fit_to = FitTo::Width(w);
    } else if let Some(h) = args.height {
        default_size = usvgr::Size::from_wh(100.0, h as f32).unwrap();
...
fn query_all_impl(parent: &usvgr::Group) -> usize {
    let mut count = 0;
    for node in parent.children() {
        if node.id().is_empty() {
            if let usvgr::Node::Group(ref group) = node {
...
fn render_svg(args: &Args, tree: &usvgr::Tree) -> Result<tiny_skia::Pixmap, String> {
    let now = std::time::Instant::now();
