...
static GLOBAL_IMAGE_DATA: Lazy<
    std::sync::Arc<HashMap<String, std::sync::Arc<usvgr::PreloadedImageData>>>,
> = Lazy::new(|| {
    let mut hash_map = HashMap::new();

    hash_map.insert(
        "../../../resources/image.png".to_owned(),
        load_image("tests/resources/image.png"),
    );
    hash_map.insert(
        "../../../resources/image.jpg".to_owned(),
        load_image("tests/resources/image.jpg"),
    );
    hash_map.insert(
...
static GLOBAL_SUB_SVGS: Lazy<std::sync::Arc<HashMap<String, Arc<usvgr::Tree>>>> = Lazy::new(|| {
    let mut hash_map = HashMap::new();

    hash_map.insert(
        "../../../resources/image.svg".to_owned(),
        load_sub_svg("tests/resources/image.svg"),
    );
    hash_map.insert(
        "../../../resources/svg-with-text.svg".to_owned(),
        load_sub_svg("tests/resources/svg-with-text.svg"),
    );
...
fn load_image(path: &str) -> Arc<usvgr::PreloadedImageData> {
...
pub fn render(name: &str) -> usize {
    ...
    opt.sub_svg_data = Some(&GLOBAL_SUB_SVGS);

    let tree = {
        let svg_data = std::fs::read(&svg_path).unwrap();
        let db = GLOBAL_FONTDB.lock().unwrap();
        usvgr::Tree::from_data(&svg_data, &opt, &db).unwrap()
    };
