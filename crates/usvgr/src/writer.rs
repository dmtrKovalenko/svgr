...
use svgrtypes::{parse_font_families, FontFamily};
...
fn write_group_element(g: &Group, is_clip_path: bool, opt: &WriteOptions, xml: &mut XmlWriter) {
    if is_clip_path {
        ...
        // The problem is that in `usvgr` we can set a `clip-path` only on groups.
        ...
        // </clipPath>
        //
        // will be represented in usvgr as:
        //
        // <clipPath id="clip">
...
impl XmlWriterExt for XmlWriter {
    ...
    fn write_image_data(&mut self, kind: &ImageKind) {
        ...
        let (mime, data) = match kind {
            ImageKind::DATA(data) => {
                self.write_attribute("xlink:href", &data.as_ref().id);
                return;
            }
