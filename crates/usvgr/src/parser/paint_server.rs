...
fn node_to_user_coordinates(
    ...
    match node {
        ...
        Node::Image(ref mut _image) => {
            // FFRAMES CHANGE
            // The commented code below process linked SVGs but in fframes
            // we process and convert all the svgs in advance so this is not needed
            // if let ImageKind::SVG(ref mut tree) = image.kind {
            //     if let Some(ref mut tree) = Arc::get_mut(tree) {
            //         update_paint_servers(
            //             &mut tree.root,
            //             context_transform,
            //             context_bbox,
