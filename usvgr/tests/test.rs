...
fn groups() {
    ...
    let tree = usvgr::Tree::from_str(&svg, &usvgr::Options::default()).unwrap();
    // Excluding root.
    let has_groups = tree
        .root
        .descendants()
        .skip(1)
        .any(|n| matches!(*n.borrow(), usvgr::NodeKind::Group(_)));
    assert!(!has_groups);
}
...
fn size_detection_1() {
    ...
    let tree = usvgr::Tree::from_str(&svg, &usvgr::Options::default()).unwrap();
    assert!(tree.size.fuzzy_eq(&usvgr::Size::new(10.0, 20.0).unwrap()));
}
...
fn size_detection_2() {
    ...
    let tree = usvgr::Tree::from_str(&svg, &usvgr::Options::default()).unwrap();
    assert!(tree.size.fuzzy_eq(&usvgr::Size::new(30.0, 40.0).unwrap()));
}
...
fn size_detection_3() {
    ...
    let tree = usvgr::Tree::from_str(&svg, &usvgr::Options::default()).unwrap();
    assert!(tree.size.fuzzy_eq(&usvgr::Size::new(5.0, 20.0).unwrap()));
}
...
fn size_detection_4() {
    ...
    ";
    let tree = usvgr::Tree::from_str(&svg, &usvgr::Options::default()).unwrap();
    assert!(tree.size.fuzzy_eq(&usvgr::Size::new(36.0, 36.0).unwrap()));
    assert!(tree
        .view_box
        .rect
        .fuzzy_eq(&usvgr::Rect::new(0.0, 0.0, 36.0, 36.0).unwrap()));
}
...
fn size_detection_5() {
    ...
    let tree = usvgr::Tree::from_str(&svg, &usvgr::Options::default()).unwrap();
    assert!(tree.size.fuzzy_eq(&usvgr::Size::new(100.0, 100.0).unwrap()));
}

#[test]
fn invalid_size_1() {
    let svg = "<svg width='0' height='0' viewBox='0 0 10 20' xmlns='http://www.w3.org/2000/svg'/>";
    let result = usvgr::Tree::from_str(&svg, &usvgr::Options::default());
