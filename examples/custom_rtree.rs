...
fn main() {
    ...
    let mut rtree = usvgr::Tree::create(usvgr::Svg {
        size,
    ...
    rtree.append_to_defs(usvgr::NodeKind::LinearGradient(usvgr::LinearGradient {
        id: "lg1".into(),
        x1: 0.0,
        y1: 0.0,
        x2: 1.0,
    ...
    let fill = Some(usvgr::Fill {
        paint: usvgr::Paint::Link("lg1".into()),
        ..usvgr::Fill::default()
    });

    rtree.root().append_kind(usvgr::NodeKind::Path(usvgr::Path {
        fill,
        data: Rc::new(usvgr::PathData::from_rect(usvgr::Rect::new(20.0, 20.0, 160.0, 160.0).unwrap())),
        .. usvgr::Path::default()
    }));
    ...
    svgr::render(&rtree, usvgr::FitTo::Original, tiny_skia::Transform::default(), pixmap.as_mut()).unwrap();
